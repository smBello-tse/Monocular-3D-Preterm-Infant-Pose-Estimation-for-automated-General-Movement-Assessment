...
from lib.utils.agma import rotation_matrix_3d
...
class ViewSampler(nn.Module):
    def __init__(self, nb_views=1):
        super(ViewSampler, self).__init__()
...
class DSTformer(nn.Module):
    def __init__(self, dim_in=3, dim_out=3, dim_feat=256, dim_rep=512,
                 ...
                 qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, att_fuse=True, train_mode="mono", nb_views=1, model_type="og"):
        #"mono" for monocular and "multi" for multi-view. if "mono" and multi_views, sample multi_views
        super().__init__()
        ...
        self.fusion_transformation = nn.Conv2d(dim_feat, dim_feat, kernel_size=1)
        self.fusion_bn = nn.BatchNorm2d(dim_feat)
        ...
        self.head_rotation = None if model_type=="og" else nn.Linear(dim_rep , 6)
        self.rotation_bn = None if model_type=="og" else nn.BatchNorm2d(6)
    ...
    def forward(self, x, return_rep=False):
        if self.train_mode == "mono" and len(x.shape) == 4:
            B, F, J, C = x.shape
            x = x.reshape(-1, J, C)
            BF = x.shape[0]
            ...
            B, V, F, J, C = x.shape
            N = V - 1  # number of source views

            # Reference view: reshape and embed
            x_ref = x[:, 0, :, :, :].reshape(-1, J, C)  # (B*F, J, C)
            x_ref = self.joints_embed(x_ref)  # (B*F, J, dim_feat)

            # Process and fuse all source views
            x_fused_total = 0
            for v in range(1, V):
                x_source = x[:, v, :, :, :].reshape(-1, J, C)  # (B*F, J, C)
                x_source = self.joints_embed(x_source)  # (B*F, J, dim_feat)
                x_source = x_source.permute(0, 2, 1).unsqueeze(-1)  # (B*F, dim_feat, J, 1)
                x_transformed = self.fusion_transformation(x_source)  # (B*F, dim_feat, J, 1)
                x_fused_total += x_transformed  # accumulate across views

            # Apply BatchNorm once after accumulating all transformed views
            x_fused = self.fusion_bn(x_fused_total)  # (B*F, dim_feat, J, 1)

            # Restore shape to (B*F, J, dim_feat)
            x_fused = x_fused.squeeze(-1).permute(0, 2, 1)  # (B*F, J, dim_feat)

            # Add to reference and apply positional encoding
            x = x_ref + x_fused  # (B*F, J, dim_feat)
            x = x + self.pos_embed  # optional: broadcast or reshape pos_embed as needed
            BF = x.shape[0]
        ...
        if self.head_rotation is not None:
            rot = self.rotation_bn(self.head_rotation(x).permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            return x_out, torch.mean(rot, dim=2)
        else:
            return x_out
