...
def loss_mpjpe_hardest(predicted, target, top_k=0.1):
    """
    Mean per-joint position error (MPJPE) computed only on the hardest joints.

    Args:
        predicted: Tensor of shape (T, J, 3)
        target: Tensor of shape (T, J, 3)
        top_k: Fraction (0 < top_k <= 1) or integer number of hardest joints to include.
               If < 1, treated as a fraction of joints.
    Returns:
        Scalar loss (mean of hardest joint errors).
    """
    assert predicted.shape == target.shape, "Predicted and target must have same shape"

    # Compute per-joint Euclidean error
    errors = torch.norm(predicted - target, dim=-1)  # (T, J)

    # Flatten across time if you want hardest joints globally
    errors_flat = errors.view(-1)  # (T*J)

    # Determine how many joints to keep
    if top_k < 1:
        k = max(1, int(errors_flat.numel() * top_k))
    else:
        k = int(top_k)

    # Select top-k hardest joints
    hardest_errors, _ = torch.topk(errors_flat, k, largest=True)

    # Return mean error for hardest joints
    return hardest_errors.mean()
