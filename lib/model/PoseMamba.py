...
class  PoseMamba(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=256, depth=6, mlp_ratio=2., drop_rate=0., drop_path_rate=0.2,  norm_layer=None, model_type="og"):
        ...
        self.head_rot = None if model_type=="og" else nn.Sequential(nn.LayerNorm(embed_dim), nn.Linear(embed_dim , 6),)
    ...
    def forward(self, x):
        ...
        if self.head_rot is not None:
            rot = self.head_rot(x)
            return x_out.view(b, f, n, -1) , torch.mean(rot, dim=2)
        else: return x_out.view(b, f, n, -1)  
