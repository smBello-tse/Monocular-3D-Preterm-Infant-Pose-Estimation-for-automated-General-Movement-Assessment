...
def worker_init_fn(worker_id):
    """Each worker loads the latest curriculum_epoch from the parent dataset at startup."""
    worker_info = torch.utils.data.get_worker_info()
    if worker_info is not None:
        dataset = worker_info.dataset
        dataset.curriculum_epoch = dataset.curriculum_epoch_global
    ...
    with torch.no_grad():
        for batches in tqdm(test_loader):
            ...
            if losses is not None:
                ...
                #loss_aligned, predicted_3d_pos_aligned = loss_p_mpjpe(predicted_3d_pos, batch_gt)
                ...
                loss_view = 0. if args.model_type=="og" else loss_mpjpe(proj[..., :2], batch_input[...,:2] - batch_input[:, :, 11:12, :2])
...
def train_epoch(args, model_pos, train_loader, losses, optimizer, has_3d, has_gt, fusion_ =False, model_view = None):
    ...
    for idx, (batches) in tqdm(enumerate(train_loader)):
        ...
        if has_3d:
            ...
            losses['total'].update(loss_total.item(), batch_size)
        with torch.autograd.set_detect_anomaly(True): loss_total.backward()
        # Clip gradients by value (between -0.5 and 0.5)
...
def train_with_config(args, opts):
    ...
    train_loader_3d = DataLoader(train_dataset, worker_init_fn=worker_init_fn, **trainloader_params)
    ...
    if not opts.evaluate:
        ...
        #optimizer = optim.SGD(filter(lambda p: p.requires_grad, model_pos.parameters()), lr=lr, weight_decay=args.weight_decay, momentum=args.momentum)
        ...
        for epoch in range(st, args.epochs):
            ...
            #if hasattr(train_loader_3d, 'worker_init_fn'):
             #   for worker_id in range(train_loader_3d.num_workers):
              #      train_loader_3d.worker_init_fn(worker_id)
            ...
            if e1 < min_loss and loss_validation[-1] < min_val_loss:
                ...
                if args.save_best: save_checkpoint(os.path.join(opts.checkpoint, f"best_epoch.bin"), best_epoch, best_lr, optimizer, best_model, min_loss)
